...
with st.sidebar:
    st.markdown("<h1 style='text-align:center;'>🏥 Hospital<br>Management</h1>", unsafe_allow_html=True)
    st.divider()
    page = st.radio(
        "Navigation",
        ["🏠 Dashboard", "👤 Patients", "👨‍⚕️ Doctors", "📅 Appointments", "📖 Medical History", "🔄 Doctor Rotation", "📊 Reports", "🔗 Linked List Viz", "⚙ Settings"],
        label_visibility="collapsed",
    )
    st.divider()
    st.caption(f"Patients: {pm.count} | Doctors: {dm.count} | Queue: {am.count}")
...
# ──── Patients ──────────────────────────────────────────────────────────
    ...
    with tab2:
        ...
        if search_term or search_btn:
            results = pm.search_by_name(search_term) if search_term else []
            if results:
...
# ──── Settings ──────────────────────────────────────────────────────────
    ...
    st.code(f"Database: hospital.db")
    st.metric("Patients in DB", db.get_table_count("patients"))
    st.metric("Doctors in DB", db.get_table_count("doctors"))
    st.metric("Appointments in DB", db.get_table_count("appointments"))

    st.divider()
    st.subheader("📋 System Info")
    st.code(f"Linked List: Patient SLL (Singly), Medical DLL (Doubly), Doctor CLL (Circular), Appointment Queue")
